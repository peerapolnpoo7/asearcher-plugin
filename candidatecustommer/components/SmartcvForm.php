...
class SmartcvForm extends ComponentBase
{
    ...
    public function onSave(){
        ...
        $messages = [
            ...
            'GPA.regex' => 'รูปแบบของ "GPA" ไม่ถูกต้อง',
        ...
        $chkImgCV=PhotoProfileCV::where('idCandidate',$candidate->idCandidate);
        if($chkImgCV->count() > 0)
        {
        ...
        $chkEdu=Education::where('idCandidate',$candidate->idCandidate);
        if($chkEdu->count() > 0){
            $education = Education::find($chkEdu->first()->idEducation);
        }else{
            $education = new Education();
        }
        $education->idCandidate = $candidate->idCandidate;
        ...
        if(Input::get('Type_Candidate')=='2'){
            $chkExp=Experience::where('idCandidate',$candidate->idCandidate);
            if($chkExp->count() > 0){
        ...
        $chkSkillSpecific=SkillSpecific::where('idCandidate',$candidate->idCandidate);
        ...
        $skillspecific->idCandidate = $candidate->idCandidate;
        ...
        $chkSkillCom=SkillBasicCom::where('idCandidate',$candidate->idCandidate);
        ...
        $skill_basic_com->idCandidate = $candidate->idCandidate;
        ...
        $chkReOfWork=RequirementOfWork::where('idCandidate',$candidate->idCandidate);
        ...
        $requirement_of_works->idCandidate = $candidate->idCandidate;
        ...
        if(post('idTransportation_Detail')){
            ...
            foreach (post('idTransportation_Detail') as $idTransportation_Detail) {
                if($idTransportation_Detail!=""){
                    ...
                    $transportation_of_work->idCandidate = $candidate->idCandidate;
